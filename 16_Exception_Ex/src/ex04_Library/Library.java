...
public class Library {
  ...
  private void modifyBook() throws RuntimeException, InputMismatchException {
    ...
    if(books.isEmpty()) {
      throw new RuntimeException("도서관에 등록되어 있는 책이 없습니다.");
    }
    String title = sc.next();
    if(title.isEmpty()) {
    ...
    System.out.println("제목을 가진 책이 없습니다.");
  }
  ...
  private void queryBook() throws RuntimeException {
    ...
    if(books.isEmpty()) {
      throw new RuntimeException("조회할 수 있는 책이 없습니다.");
    }
    ...
    for(Book book : books) {
      if(title.equals(book.getTitle())) {
        System.out.println(book + "이 조회되었습니다.");
        return;
      }
    }
    System.out.println(title + " 제목을 가진 책이 없습니다.");
  ...
  /**
   * 전체조회<br>
   * List<Book> book의 모든 Book 객체를 순서대로 출력하는 메소드
