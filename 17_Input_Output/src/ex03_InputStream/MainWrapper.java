...
public class MainWrapper {
   ...
   * 1. 바이트 기반의 입력스트림이다.
   * 2. 입력 단위
   *    1) int      (하나씩  읽을 거면 int    써라)
   *    2) byte[]   (여러 개 읽을 거면 byte[] 써라)
   */
  
  public static void ex01() {
    
    // 디렉터리를 파일 객체로 만들기
    // 읽어야 될 객체가 있는 곳이니 같은 저장 장소.
    ...
    try {
      ...
      /*
      c = fin.read();  // 가장 기본 : 데이터 하나 읽어와서 c에 저장. ( Apple 중 A만 읽어옴 ) 
      sb.append(c);    // 위에서 한 글자 읽어서 StringBuilder에 저장.
      c = fin.read();
      .
      .
      5번 작업해야 Apple 이 돼서, 반복문으로 작업한다.
      */
      while( (c = fin.read()) != -1 ){
        sb.append((char)c);   // 미리 character 로 변환.
      }
      
      // 결과 확인
      System.out.println(sb.toString());
      // StringBuilder 를 써서 toString 해줌.
  ...
  public static void ex02() {
    ...
    try {
      ...
      // 실제로 읽은 바이트 수 저장 변수 
      // (몇 바이트 저장했는지 알아야 함, 배열이라 덮어쓰기 되면 기존과 다른 값이 나올 수 있기 때문, ex: 15바이트 = 4, 4, 4, 3바이트.)
      int readByte = 0;
      ...
      readByte = fin.read(b); // 실제 저장된 데이터는 b에 있고, readByte 에는 바이트 수가 들어감.
  ...
  public static void ex03() {
    ...
    try {
      ...
      readByte = bin.read(b); // 실제 저장된 데이터는 b에 있고, readByte 에는 바이트 수가 들어감.
      sb.append(new String(b, 0, readByte));
